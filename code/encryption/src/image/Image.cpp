...
Image::Image(string path,string ext){
  ...
  this->code = false;
  this->decode = true;
  if(ext==".jpg"){	//OPEN JPEG for recompression
    Image::toMem(path);
...
void Image::compress_JPG(std::string pathOut,int qf,bool dc,bool ac,bool shuffle,bool FIBS,bool xorOp,bool lum,bool crcb,unsigned long seed){
  ...
  jpeg_set_quality(&cinfo, qf, true);
...
void Image::cryting(std::string pathOut,unsigned long seed){
  ...
  if(shuffle && !code){ //false decode
    for(int k=0;k<_spectrum;k++){	//pas optimis\E9
	...
	for(bx=0;bx<compptr_one->width_in_blocks;bx++){
	  ...
	  for(bi=start_Coef;bi<blck_size;bi++){//coefs
	    if(FIBS){
	      ...
	      if(recomp==0){//chiffre
		if(!FIBS && blockptr[bi]!=0 && blockptr[bi]!=1 && blockptr[bi]!=-1 ){ // pas 1 -1 pour deshuffle sinon les recompress\E9 pas deshuffle
		  blockptr[bi]  = cdct[ci][bi][res[ci][bi][cpt2[ci][bi]]];
		  cpt2[ci][bi]++;
		}
	      }else{
  ...
  if(xorOp){
	...
	for(bx=0;bx<compptr_one->width_in_blocks;bx++){
	  ...
	  for(bi=start_Coef;bi<blck_size;bi++){	//AC ou DC
						//if(blockptr[bi]!=0 && ((!recomp&& blockptr[bi]!=-1024) || (recomp&& blockptr[bi]!=-512))){	//chiffre que les coefs non nuls et pas de taille 1 : /2 ->0 //original 1024 cod\E9 11 bits pb donc pas de chiffrement
	    if(blockptr[bi]!=0){	
	      if(recomp==0){//chiffre
	...
	for(bx=0;bx<compptr_one->width_in_blocks;bx++){
	  ...
	  for(bi=start_Coef;bi<blck_size;bi++){//AC
	    if(FIBS){
	      ...
	      if(blockptr[bi]!=0 && blockptr[bi]!=1 && blockptr[bi]!=-1 ){ // pas 1 -1 pour deshuffle sinon les recompress\E9 pas deshuffle
		blockptr[bi]  = cdct[ci][bi][indices[ci][bi][cpt2[ci][bi]]];
		cpt2[ci][bi]++;
	      }
	    }
...
int Image::write_jpeg_file(std::string outname,jpeg_decompress_struct in_cinfo, jvirt_barray_ptr *coeffs_array){
  struct jpeg_compress_struct cinfo;
  struct jpeg_error_mgr jerr;
  FILE * infile;

  int Q_base[DIM_BLOCK*DIM_BLOCK]={16,11,10,16,24,40,51,61,12,12,14,19,26,58,60,55,14,13,16,24,40,57,69,56,14,17,22,29,51,87,80,62,18,22,37,56,68,109,103,77,24,35,55,64,81,104,113,92,49,64,78,87,103,121,120,101,72,92,95,98,112,100,103,99};
  double sf1 = 0;
  double sf2 = 0;

  cout << "opening " << outname << "\n";
  ...
  JQUANT_TBL* quant_ptr = cinfo.quant_tbl_ptrs[0];
  for (int i = 0; i < DIM_BLOCK*DIM_BLOCK; i++) {
    uint tmp = quant_ptr->quantval[i];
    sf1 += 100 - ((50*tmp-25)/Q_base[i]);
    sf2 += (5000*Q_base[i])/(100*tmp-50);
  }
  ...
  //calcul qf approx luminance
	
  sf1 /= DIM_BLOCK*DIM_BLOCK;
  sf2 /= DIM_BLOCK*DIM_BLOCK;
  int qf;
  if(sf2 < 50)
    qf = sf2;
  else
...
int Image::exportRecomp_JPG(std::string pathOut){
  recomp = 1;
  this->decode = false;
  //load
  struct jpeg_decompress_struct cinfo;
  ...
  //export
  int nQf = write_jpeg_file(pathOut, cinfo, coeffs_array);
