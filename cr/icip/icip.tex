...
\label{sec:intro}
More and more content is being shared everyday on the internet. Most of this content is trivial and does not need to be encrypted. Some of it however needs to be securely transfered, the problem of encryption arises. Full encryption with methods such as AES for exemple are often not needed in addition to not being possible due to computing power constraints. Instead, partial or selective encryption is used, where the goal is sufficient encryption. That is, the image is sufficently distorted and an attacker is not able to access the content. This distortion can be of varying magnitude, a strong distortion for exemple for DRM, or a lighter distortion, where the content is still recognizable to attract the viewers interest. \textit{TODO : encryption state of the art}. When an image is intented to be consumed by a human, the most accurate measure of its confidentiality is a Mean Opinion Score, where acte people rate the image. It is however not a realistic way to rate the distortion of an image as it is way to expensive and time consuming, security and quality metrics were introduced as a means to automate the process. 

% As it often useless to use traditionnal encryption method such as DES or AES for exemple on visual content, as well as not always possible due to computing power constraints, we use encryption methods specifically devised for visual content. The idea is that when an image is sufficiently distorted, its content is protected. Our end goal in is to automatically rate an image level of protection, and to decide whether its content is confidential. We focus on JPEG images as it is the most widely used lossy compression standard. Metrics such as PSNR, entropy, or even SSIM are often used as benchmarks for visual content confidentiality \cite{vsm_deficits}. Such metrics however are designed for high quality image, and lack consistency in the low end of Mean Opinion Scores \cite{vsm_deficits}.
